...
export default function UpdateFeedbacks(self) {
  ...
  self.setFeedbackDefinitions({
    outputRouted: {
      ...
      description:
        "Highlights the button when the chosen output is currently routed to the chosen source or scene.",
      ...
      callback: (feedback) => {
        const output = self.state?.outputs.find(
          (o) => o.id === feedback.options.outputId,
        );
        return (
          !!output &&
          output.routedSourceId === (feedback.options.targetId || null)
        );
      },
    },
    clientOnline: {
      ...
      description:
        "Highlights the button while the chosen Client Node is connected.",
      defaultStyle: { bgcolor: 0x00aa00, color: 0xffffff },
      options: [
        {
          id: "clientId",
      ...
      callback: (feedback) => {
        const client = self.state?.clients.find(
          (c) => c.id === feedback.options.clientId,
        );
